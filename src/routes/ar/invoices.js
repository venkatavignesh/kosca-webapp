...
router.get('/ar/invoices', requireAuth, requireInvoicesAccess, async (req, res) => {
    ...
    const groups_param = req.query.group
        ? (Array.isArray(req.query.group) ? req.query.group : [req.query.group]).filter(Boolean)
        : [];
    ...
    try {
        ...
        const assignedCodeFilter = finalCodes !== null
        ...
        if (isNameSort) {
            ...
            const sortNameMap = {};
            sortNameMasters.forEach(cm => { if (cm.customerName) sortNameMap[cm.customerCode] = cm.customerName; });
        ...
        const inMemorySortFns = {
            ...
            'name_asc':     (a, b) => (a.customerName||a.customerCode).toLowerCase().localeCompare((b.customerName||b.customerCode).toLowerCase()),
            'name_desc':    (a, b) => (b.customerName||b.customerCode).toLowerCase().localeCompare((a.customerName||a.customerCode).toLowerCase()),
        };
        ...
        grouped.forEach(g => { g.siteNames = Array.from(g.siteNames); });
        ...
        const customerMasters = await prisma.customerMaster.findMany({
            ...
            select: { customerCode: true, mobileNo: true, phone: true, mobileNoLocked: true, masterMobileNo: true, psrName: true }
        });
        const mobileMap = {};
        customerMasters.forEach(cm => {
            mobileMap[cm.customerCode] = { number: cm.mobileNo || cm.phone || null, locked: cm.mobileNoLocked, masterMobileNo: cm.masterMobileNo, psrName: cm.psrName || null };
        });
        grouped.forEach(g => {
        ...
        grouped.forEach(g => { g.assignedOfficerIds = assignmentMap[g.customerCode] || []; });
        ...
        grouped.forEach(g => { g.isDisputed = disputedSet.has(g.customerCode); });
        ...
        if (canPS) {
            ...
            grouped.forEach(g => { g.hasPendingSettlement = psSet.has(g.customerCode); });
        }
        ...
        const groupNameMap = {};
        groupRecords.forEach(r => { groupNameMap[r.customerCode] = r.groupName; });
        grouped.forEach(g => { g.groupName = groupNameMap[g.customerCode] || null; });
...
router.get('/ar/invoices/:customerCode', requireAuth, requireModule('ar_directory'), async (req, res) => {
    try {
        const sitesParam = req.query.site
            ? (Array.isArray(req.query.site) ? req.query.site : [req.query.site])
            : [];
        const bucketRanges = {
            '0_30':    { lte: 30 },
