...
router.get('/ar/directory', requireAuth, requireModule('ar_directory'), async (req, res) => {
    ...
    const selectedSites = req.query.site
        ? (Array.isArray(req.query.site) ? req.query.site : [req.query.site])
        : [];
    ...
    const allSiteNames = siteNameRows.map(r => r.siteName).filter(Boolean).sort();
    const keyOnly = req.query.keyOnly === '1';
    const subOnly = req.query.subOnly === '1';
    const psOnly  = req.query.ps_only === '1';

    const selectedPsrs = req.query.psr
        ? (Array.isArray(req.query.psr) ? req.query.psr : [req.query.psr])
        : [];

    // Build PSR → sites map for location-dependent client-side filtering
    const psrMasters = await prisma.customerMaster.findMany({
        where: { psrName: { not: null } },
        select: { customerCode: true, psrName: true }
    });
    const psrCodeMap = {};
    psrMasters.forEach(cm => { psrCodeMap[cm.customerCode] = cm.psrName; });
    const psrCodes = Object.keys(psrCodeMap);
    const sitePsrRows = psrCodes.length ? await prisma.invoice.findMany({
    ...
    const allPsrData = Object.entries(psrSiteMap)
    ...
    const groupSiteRows = allGroupCodes.length ? await prisma.invoice.findMany({
    ...
    const allGroupData = Object.keys(groupCodeMap).sort().map(gn => {
        const sites = new Set();
    ...
    const groupCount = allGroupData.length;
    const selectedGroup = req.query.group || '';

    res.render('ar/directory', { bucket, selectedSites, allSiteNames, keyOnly, subOnly, psOnly, groupCount, allGroupData, selectedGroup, allPsrData, selectedPsrs });
});
...
router.get('/ar/groups-data', requireAuth, requireModule('ar_directory'), async (req, res) => {
    ...
    try {
        ...
        if (tab === 'ungrouped') {
            const [keyAccountCodes, groupedRows, allCustRows] = await Promise.all([
                ...
                prisma.invoice.findMany({ where: { status: 'ACTIVE' }, select: { customerCode: true }, distinct: ['customerCode'] })
            ]);
            ...
            const ungroupedCodes = allCustRows.map(r => r.customerCode).filter(c => !excludeSet.has(c)).sort();
            if (ungroupedCodes.length === 0) {
                return res.send('<div class="py-12 text-center text-sm text-gray-400">All non-key customers have been assigned to a group.</div>');
            }
            const [invSummary, masterRows, allGroups] = await Promise.all([
                prisma.invoice.groupBy({
                    by: ['customerCode'],
                    where: { customerCode: { in: ungroupedCodes }, status: 'ACTIVE' },
                    _count: { _all: true }, _sum: { balanceAmount: true }, _max: { agingDays: true }
                }),
                prisma.customerMaster.findMany({
                    where: { customerCode: { in: ungroupedCodes } },
                    select: { customerCode: true, customerName: true }
                }),
                isAdmin ? prisma.customerGroup.findMany({ select: { groupName: true }, distinct: ['groupName'], orderBy: { groupName: 'asc' } }) : []
            ]);
            const invMap = {};
            ...
            const agingBadge = (d) => {
                if (!d) return '<span class="inline-flex items-center justify-center w-14 text-xs text-gray-300">—</span>';
                const cls = d > 90 ? 'bg-red-50 text-red-700 border-red-200' : d > 60 ? 'bg-orange-50 text-orange-700 border-orange-200' : d > 30 ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-blue-50 text-blue-700 border-blue-200';
            ...
            const groupOptions = allGroups.map(g => `<option value="${g.groupName.replace(/"/g,'&quot;')}">${g.groupName.replace(/</g,'&lt;')}</option>`).join('');
            let rowsHtml = '';
            for (const code of ungroupedCodes) {
                ...
                rowsHtml += `<tr class="border-b border-gray-100 last:border-0 odd:bg-white even:bg-gray-50/60 hover:bg-gray-100/80 cursor-pointer" onclick="window.location.href='/ar/invoices/${encodeURIComponent(code)}'">
  <td class="px-2 py-1.5 whitespace-nowrap"><span class="text-[11px] text-gray-400 font-mono">${escHtml(code)}</span></td>
  <td class="px-2 py-1.5"><span class="text-xs font-semibold text-gray-900">${name}</span></td>
  <td class="px-2 py-1.5 text-center whitespace-nowrap"><span class="text-xs font-bold text-gray-900">${fmt(balance)}</span></td>
  <td class="px-2 py-1.5 text-center whitespace-nowrap"><span class="text-[11px] text-gray-400">${count}</span></td>
  ...
  ${isAdmin ? `<td class="px-3 py-2.5 whitespace-nowrap" onclick="event.stopPropagation()">
    <div x-data="{ grp: '' }" class="flex gap-1.5 items-center">
      <select x-model="grp" class="rounded-lg border-0 py-1 pl-2 pr-6 text-xs ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-indigo-400 bg-white appearance-none">
      ...
      <button type="button" :disabled="!grp" data-code="${code}"
        ...
        class="px-2 py-1 rounded-lg bg-indigo-50 text-indigo-600 text-xs font-semibold hover:bg-indigo-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Assign</button>
    </div>
  </td>` : ''}
</tr>`;
            }
            return res.send(`<div class="overflow-x-auto">
  <div class="px-4 py-2 border-b border-gray-100 bg-gray-50/60 flex items-center justify-between">
    <span class="text-xs text-gray-500"><span class="font-bold text-gray-800">${ungroupedCodes.length}</span> ungrouped customer${ungroupedCodes.length !== 1 ? 's' : ''}</span>
  </div>
  <table class="w-full border-collapse text-sm">
    <thead>
      <tr class="border-b border-gray-200 bg-gray-50/80">
        <th class="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Code</th>
        <th class="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Customer</th>
        <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Outstanding</th>
        <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Inv</th>
        <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Aging</th>
        ${isAdmin ? '<th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Assign</th>' : ''}
      </tr>
    </thead>
    <tbody>${rowsHtml}</tbody>
        ...
        if (groupEntries.length === 0) {
            return res.send('<div class="py-12 text-center text-sm text-gray-400">No grouped customers for this category. Use Group Import to add groups.</div>');
        }
        ...
        const invoiceMap = {};
        for (const r of invSummary) invoiceMap[r.customerCode] = { count: r._count._all, balance: r._sum.balanceAmount || 0, maxAging: r._max.agingDays || 0 };
        ...
        html += '<thead><tr class="border-b border-gray-200 bg-gray-50/80">';
        html += '<th class="w-7 px-2 py-2"></th>';
        html += '<th class="px-2 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Group</th>';
        html += '<th class="px-2 py-2 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Doors</th>';
        html += '<th class="px-2 py-2 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Outstanding</th>';
        html += '<th class="px-2 py-2 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Invoices</th>';
        html += '<th class="px-2 py-2 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Max Aging</th>';
        ...
        for (const grp of Object.keys(groupMap).sort()) {
            ...
            for (const code of codes) {
                subIdx++;
                const subRowBase = subIdx % 2 === 0 ? 'bg-gray-100' : 'bg-white';
                ...
                rowsHtml += `<tr class="${subRowBase} border-b border-gray-100 last:border-0 hover:bg-indigo-50 cursor-pointer transition-colors" onclick="window.location.href='/ar/invoices/${encodedCode}'">
  <td class="px-2 py-1.5 whitespace-nowrap"><span class="text-[11px] text-gray-400 font-mono">${escHtml(code)}</span></td>
  <td class="px-2 py-1.5"><span class="text-xs font-semibold text-gray-900 truncate block max-w-[200px]" title="${name.replace(/"/g,'&quot;')}">${name.replace(/</g,'&lt;')}</span></td>
  <td class="px-2 py-1.5 text-center">${m?.psrName ? `<span class="inline-flex items-center rounded-md px-1 py-px text-[10px] font-medium leading-tight bg-green-50 text-green-700 border border-green-200">${m.psrName.replace(/</g,'&lt;')}</span>` : '<span class="text-[11px] text-gray-300">—</span>'}</td>
  <td class="px-2 py-1.5 text-center whitespace-nowrap"><span class="text-xs font-bold text-gray-900">${fmt(balance)}</span></td>
  <td class="px-2 py-1.5 text-center whitespace-nowrap"><span class="text-[11px] text-gray-400">${count}</span></td>
  ...
  ${isAdmin ? `<td class="px-2 py-1.5 text-center whitespace-nowrap" onclick="event.stopPropagation()">
    <button type="button"
      hx-post="/admin/groups/member/${encodedCode}/remove"
      hx-target="closest tr" hx-swap="outerHTML"
      hx-confirm="Remove ${safeName} from this group?"
      onclick="event.stopPropagation()"
      class="p-1 rounded text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors" title="Remove from group">
  ...
  <tr class="border-b border-violet-200 cursor-pointer select-none transition-colors hover:bg-violet-100/60 bg-violet-50" :class="open ? '!bg-violet-100/60 !border-b-2 !border-violet-300' : ''" @click="open = !open">
    <td class="px-2 py-2 w-7">
      <div class="flex items-center justify-center text-violet-400 transition-colors" :class="open ? 'text-violet-600' : ''">
    ...
    <td class="px-2 py-2">
      <span class="text-xs font-bold text-violet-900">${safeGrp}</span>
    </td>
    <td class="px-2 py-2 text-center whitespace-nowrap"><span class="text-[11px] text-gray-500">${codes.length}</span></td>
    <td class="px-2 py-2 text-center whitespace-nowrap"><span class="text-xs font-bold text-violet-900">${fmt(totalBalance)}</span></td>
    <td class="px-2 py-2 text-center whitespace-nowrap"><span class="text-[11px] text-gray-400">${totalCount}</span></td>
  ...
  <tr x-show="open" x-cloak class="border-b-2 border-violet-100">
    <td colspan="${colSpan}" class="p-0 px-3 pb-3">
      <div class="border-l-3 border-violet-300 bg-slate-50/60 rounded-b-lg overflow-hidden">
      <table class="w-full border-collapse text-sm">
        <thead>
          <tr class="border-b border-gray-200 bg-white/70">
            <th class="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Code</th>
            <th class="px-2 py-1.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Customer</th>
            <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">PSR</th>
            <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Outstanding</th>
            <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Inv</th>
            <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Aging</th>
        ...
        <tbody>${rowsHtml}</tbody>
        ${isAdmin ? `<tfoot><tr><td colspan="${subColSpan}" class="px-4 py-2 border-t border-gray-100">
          <div x-data="{ addCode: '', addSaving: false }" class="flex gap-1.5 items-center">
            <input x-model="addCode" placeholder="Add customer code…" autocomplete="off"
              class="flex-1 rounded-lg border-0 py-1.5 pl-3 pr-3 text-xs text-gray-900 ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-indigo-400 bg-white placeholder:text-gray-400"/>
            <button type="button" :disabled="addSaving || !addCode.trim()"
              ...
              class="px-2.5 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 text-xs font-semibold hover:bg-indigo-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
